...
# Aggiungi un ritardo tra i tentativi di accesso
def delayed_login(user: UserLogin):
    # Verifica il numero di tentativi falliti per l'utente
    failed_attempts = storage.get_failed_login_attempts(user.username)

    # Verifica il timestamp dell'ultimo tentativo fallito
    last_failed_attempt = storage.get_last_failed_login_attempt(user.username)

    # Verifica se l'account è temporaneamente bloccato
    if failed_attempts >= MAX_FAILED_ATTEMPTS:
        if time.time() - last_failed_attempt < BLOCKED_TIME:
            raise AccountTemporarilyBlockedException

    # Verifica le credenziali
    info = storage.user_auth(user.username, user.password)
    if info is None:
        # Aggiorna i tentativi falliti e il timestamp dell'ultimo tentativo
        storage.update_failed_login_attempts(user.username, failed_attempts + 1)
        storage.update_last_failed_login_attempt(user.username, time.time())
        
        raise InvalidCredentialsException

    # Resetta i tentativi falliti
    storage.reset_failed_login_attempts(user.username)
    ...
    return resp
BLOCKED_TIME = 300  # Tempo di blocco in secondi (5 minuti)
MAX_FAILED_ATTEMPTS = 2  # Numero massimo di tentativi falliti consentiti
...
def login(user: UserLogin):
    ...
    if info is None:
        ...
        # Controlla se il numero di tentativi supera il limite massimo
